---
title: "msigdbR: The molecular signatures database (MSigDB) in R"
...
msigdb.v7.1.SYM = eh[['EH4692']]
msigdb.v7.1.SYM
```

```{r download-msigdb-ezid}
msigdb.v7.1.EZID = eh[['EH4693']]
msigdb.v7.1.EZID
```

Alternatively, data can be downloaded using accessors in the `msigdbR` package as below:

```{r alt-download}
#metadata are displayed
msigdb.v7.1.SYM(metadata = TRUE)
#data are loaded
msigdb.v7.1.SYM()
#data are loaded
msigdb.v7.1.EZID()
...
table(sapply(lapply(msigdb.v7.1.SYM, collectionType), bcCategory))
#calculate the number of signatures in each subcategory
table(sapply(lapply(msigdb.v7.1.SYM, collectionType), bcSubCategory))
#plot the distribution of sizes
hist(sapply(lapply(msigdb.v7.1.SYM, geneIds), length),
