...
msigdb.hs.v7.1.SYM = eh[['EH4697']]
msigdb.hs.v7.1.SYM
```

```{r download-msigdb-ezid}
msigdb.hs.v7.1.EZID = eh[['EH4698']]
msigdb.hs.v7.1.EZID
```
...
msigdb.hs.v7.1.SYM(metadata = TRUE)
#data are loaded
msigdb.hs.v7.1.SYM()
#data are loaded
msigdb.hs.v7.1.EZID()
...
table(sapply(lapply(msigdb.hs.v7.1.SYM, collectionType), bcCategory))
#calculate the number of signatures in each subcategory
table(sapply(lapply(msigdb.hs.v7.1.SYM, collectionType), bcSubCategory))
#plot the distribution of sizes
hist(sapply(lapply(msigdb.hs.v7.1.SYM, geneIds), length),
