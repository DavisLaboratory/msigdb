...
`edgeR`包提供了过滤所需的数据标准化与转换方法。这些方法要求数据存储在DGEList对象中，因此我们需要将SummarizedExperiment对象转换成DGEList对象。最初因为数据矩阵中包含重复的行名（176个）而无法转换成DGEList对象。原始数据中并没有任何重复的特征，因此可能是在`GDCprepare`函数这一步发生了错误。检查后我们发现特征名称相同的数据具有相同的count值。
...
本分析流程中，在大多数样本里count数很低的基因被丢弃。这一步骤往往是差异表达分析的标准步骤，因为这些基因使得对离差（dispersion）的估计发生偏离。
