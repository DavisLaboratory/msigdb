...
`edgeR`包提供了过滤所需的数据标准化与转换方法。这些方法要求数据存储在DGEList对象中，因此我们需要将SummarizedExperiment对象转换成DGEList对象。一开始我们发现数据矩阵中包含重复的行名（176个）因而无法转换成DGEList对象。但实际上原始数据中不包含任何重复的特征，因此可能是在`GDCprepare`函数这一步发生了错误，检查后发现特征名称相同的数据具有相同的count值。
