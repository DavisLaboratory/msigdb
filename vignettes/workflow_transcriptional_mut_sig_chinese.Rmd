...
癌症是一种异质性疾病，其具有多种临床和病理亚型。以乳腺癌为例，临床上主要依靠激素受体（雌激素受体：ER；孕酮受体：PR）的表达或Erb-B2受体酪氨酸激酶（*HER2*）的过度表达进行分型，这些特征拥有直接靶向治疗的药物。许多癌症的亚型分类主要依赖于识别大型患者队列中的复发性突变（recurrent mutations），通过全基因组或全外显子组测序来获得具有临床意义的亚型 [@cancer13, @papaemmanuil16]。

尽管基因组测序得到的突变或融合基因对于肿瘤亚型分类十分重要，我们仍然很难弄清携带有这些突变的细胞的状态或表型。微阵列（microarray）技术的发展和RNA测序技术的应用则使得我们可以分析生物样本的表达谱（transcriptional profile），掌握更多细胞的表型信息[@cieslik18]。过去转录组分析主要聚焦在不同样本之间基因的差异表达，而随着公开RNA数据的快速增长，越来越多的人开始使用“relative approaches”来量化样本与gene signatures之间的一致性。最常见的例子就是临床上PAM50 signatures被用来预测乳腺癌病人的亚型 [@parker09, @cieslik18]。
...
Precision for predictions of all genomic changes other than NPM1c is undefined because all samples are predicted to be wild-types. Consequently, their recall is zero. Precision, recall and the F1 score of the NPM1c mutation model are high as expected. As mentioned in Section \@ref(score-samples), singscores are two-components scores , therefore, performance may be further improved by including the dispersion of ranks. This is observed in Figure \@ref(fig:plot-dispersion).
