...
We now use the @ross04 MLL-fusion signatures to score the TCGA AML samples. Unlike the NPM1c signature, this signature is composed of genes that discriminate MLL-fusions and PTDs from wild-type. We download and parse the signature as demonstrated in \@ref(prepare-signature).
...
Figure \@ref(fig:plot-project-mll) shows that the MLL fusions and MLL PTDs exhibit variation across different axes. MLL PTDs have a lower score than MLL fusions for the the MLL fusion signature as expected. These sets of samples do not cluster along the two axes as shown by @ross04. Overlaying samples with annotations onto the plot assists in interpreting different regions of the landscape.
...
Figure \@ref(fig:plot-project-pmlrara) shows a completely different landscape from what was observed with the MLL fusion signature. The PML-RARA signature forms a clear separation between the PML-RARA and NPM1c signatures. PML-RARA samples are the only ones with a high score for this signature. Moveover, no association is observed between the two signatures. In fact, the L-shaped landscape may suggest mutual exclusivity between the underlying biological mechanisms of these two mutations.
