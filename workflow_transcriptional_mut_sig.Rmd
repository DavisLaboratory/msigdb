...
For this analysis we have used the curated mutation list from the original TCGA AML publication [DOI: 10.1056/NEJMoa1301689] (above) rather than variant calls from the standard TCGA pipeline (available through the [National Cancer Institute Genomic Data Commons](https://gdc.cancer.gov/)) and readers should note that there are some discrepancies between these. For genetic lesions of interest (NPM1c, KMT2A-MLL, KMT2A-PTD and PML-RARA), patients were identified by the following criteria:
...
```{r plot-boxplot-mutscores, fig.wide=TRUE, fig.height=5, fig.cap="NPM1c signature scores are able to separate mutants from wilt-types. Boxplot of scores from the NPM1c signature split by different types of mutations."}
